...
export default function FeaturedProject({
  ...
  const overall = getCosplayProgressPercent(cosplay);
  const cover = buildCover(cosplay);
  const showOutfit = cosplay.outfit && cosplay.outfit.toLowerCase() !== "default";

  return (
    <div id="dashboard" className="animate-fade-up grid items-start gap-3 lg:grid-cols-[minmax(0,1.55fr)_minmax(240px,0.9fr)] lg:gap-4">
      {/* Current Build — horizontal card, ref-style */}
      <Link
        href={`/roster/${cosplay.id}`}
        className="group relative w-full overflow-hidden rounded-[1.75rem] border border-closet-pink/35 bg-white shadow-closet transition-all duration-300 hover:-translate-y-0.5 hover:border-closet-rose/25 hover:shadow-closet-lg"
      >
        {/* Character ref — right side, fades into white */}
        <div className="pointer-events-none absolute inset-y-0 right-0 w-[42%] overflow-hidden sm:w-[38%]">
          {cover ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-white from-0% via-white/25 via-[18%] to-transparent to-[38%]" />
        </div>

        {/* Info + progress */}
        <div className="relative z-10 flex flex-col p-4 pr-[44%] sm:p-5 sm:pr-[40%]">
          ...
          <div className="mt-3 h-2.5 w-full max-w-xs overflow-hidden rounded-full bg-closet-blush sm:max-w-sm">
            <div
          ...
          <span className="mt-2 text-xs font-semibold text-closet-rose/80 transition-transform group-hover:translate-x-0.5 sm:text-sm">
            Open board →
          </span>
        </div>
      </Link>
      ...
      {/* Side cards */}
      <div className="animate-stagger grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
        <article className="cosplan-side-card group">
          ...
          <div className="mt-3 flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h3 className="font-sans text-xl font-bold text-closet-brown">
                {nextCon?.title ?? "TBD"}
              </h3>
              {nextCon && (
                <>
                  <p className="mt-0.5 text-sm text-closet-brown-light">
                    {formatCalendarEventDates(nextCon)}
                  </p>
                  {nextCon.location && (
          ...
          <Link
            href="/calendar"
            className="mt-auto flex items-center justify-between border-t border-closet-pink/35 pt-4 text-sm font-semibold text-closet-rose transition-colors hover:text-closet-mauve"
          >
            View Details
